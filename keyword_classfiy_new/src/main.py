...
from workflow_ui import WorkflowUI  # 导入新的工作流UI
...
class MainWindow(tk.Tk):
    ...
    def init_ui(self):
        ...
        self.notebook.add(workflow_frame, text="工作流处理")
        
        # 新增工作流规则处理页面
        workflow_rules_frame = ttk.Frame(self.notebook, padding="10")
        self.notebook.add(workflow_rules_frame, text="工作流规则处理")
        ...
        # 初始化工作流功能UI
        self.init_workflow_ui(workflow_frame)
        
        # 初始化工作流规则处理UI
        self.init_workflow_rules_ui(workflow_rules_frame)
    ...
    def init_workflow_ui(self, main_frame):
        # 文件选择区域
        file_select_frame = ttk.LabelFrame(main_frame, text="工作流文件选择:")
        file_select_frame.pack(fill=tk.X, padx=5, pady=5)
        ...
        # 选择工作流文件按钮
        self.select_workflow_btn = ttk.Button(buttons_frame, text="选择工作流文件", command=self.select_workflow_file)
        self.select_workflow_btn.pack(side=tk.LEFT, padx=5)
        
        # 选择规则文件按钮
        self.select_rules_btn = ttk.Button(buttons_frame, text="选择规则文件", command=self.select_rules_file)
        ...
        # 工作流文件信息
        self.workflow_file_label = ttk.Label(info_frame, text="工作流文件: 未选择")
        self.workflow_file_label.pack(anchor=tk.W, padx=5, pady=2)
        
        # 规则文件信息
        self.workflow_rules_label = ttk.Label(info_frame, text="规则文件: 未选择")
        self.workflow_rules_label.pack(anchor=tk.W, padx=5, pady=2)
        
        # 工作流说明区域
        help_frame = ttk.LabelFrame(main_frame, text="工作流说明:")
        help_frame.pack(fill=tk.X, padx=5, pady=5)
        
        help_text = """工作流处理说明:
1. 工作流文件必须是Excel文件，文件名以'工作流_'开头
2. 按照sheet的创建顺序依次执行分词
3. 第一个sheet的分词结果将形成不同的Excel文件
4. 第二个sheet的分词结果将形成不同的sheet
5. 第三个及以上sheet的分词结果将形成不同的列
6. 输出结果保存在./工作流结果目录下
7. 文件名格式为: sheet1词根名称_时间.xlsx
8. 特殊字符替换规则: <>替换为《》, |替换为~, []替换为【】"""
        ...
        self.workflow_status_label = ttk.Label(status_frame, text="就绪")
        self.workflow_status_label.pack(anchor=tk.W)
        
        # 错误信息显示区域
        error_frame = ttk.LabelFrame(main_frame, text="处理信息:")
        error_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        self.workflow_info_text = scrolledtext.ScrolledText(error_frame, height=10, wrap=tk.WORD)
        self.workflow_info_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.workflow_info_text.config(state=tk.DISABLED)  # 初始状态为禁用，防止用户编辑
    ...
    def select_workflow_file(self):
        ...
        if file_path:
            ...
            if not file_name.startswith("工作流_"):
                messagebox.showwarning("警告", "工作流文件名必须以'工作流_'开头")
                return
            
            self.workflow_file_path = file_path
            self.workflow_file_label.config(text=f"工作流文件: {file_name}")
            self.add_workflow_info(f"已选择工作流文件: {file_name}")
            self.workflow_status_label.config(text="已选择工作流文件")
    ...
    def select_rules_file(self):
        ...
        if file_path:
            self.rules_file_path = file_path
            file_name = os.path.basename(file_path)
            self.workflow_rules_label.config(text=f"规则文件: {file_name}")
            self.add_workflow_info(f"已选择规则文件: {file_name}")
    
    def add_workflow_info(self, message):
        """添加工作流处理信息"""
        self.workflow_info_text.config(state=tk.NORMAL)
        self.workflow_info_text.insert(tk.END, f"{message}\n")
        self.workflow_info_text.see(tk.END)  # 自动滚动到最新消息
        self.workflow_info_text.config(state=tk.DISABLED)
    
    def clear_workflow_info(self):
        """清空工作流处理信息"""
        self.workflow_info_text.config(state=tk.NORMAL)
    ...
    def process_workflow(self):
        ...
        if not self.workflow_file_path:
            messagebox.showwarning("警告", "请先选择工作流文件")
            return
        
        if not self.rules_file_path:
            messagebox.showwarning("警告", "请先选择规则文件")
            return
        
        try:
            # 清空处理信息
            self.clear_workflow_info()
            
            # 更新状态
            self.workflow_status_label.config(text="正在处理工作流...")
            self.update()  # 更新UI
            
            # 添加处理信息
            self.add_workflow_info("开始处理工作流...")
            
            # 记录开始时间
            start_time = time.time()
            
            # 调用ExcelHandler的process_workflow方法处理工作流
            output_dir = self.excel_handler.process_workflow(
                self.workflow_file_path, 
                self.rules_file_path
            )
            ...
            # 更新状态信息
            self.workflow_status_label.config(text=f"工作流处理完成，结果已保存至 {os.path.abspath(output_dir)}")
            self.add_workflow_info(f"工作流处理完成，总耗时: {total_time:.2f}秒")
            self.add_workflow_info(f"结果已保存至: {os.path.abspath(output_dir)}")
            ...
            self.add_workflow_info(f"错误: {error_msg}")
            messagebox.showerror("错误", error_msg)
            self.workflow_status_label.config(text="处理失败")

    def init_workflow_rules_ui(self, main_frame):
        """初始化工作流规则处理UI"""
        # 创建WorkflowUI实例并添加到主框架
        workflow_ui = WorkflowUI(main_frame)
        workflow_ui.pack(fill=tk.BOTH, expand=True)
