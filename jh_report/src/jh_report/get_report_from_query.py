...
def create_start_date(date_range) -> str:
    if date_range == "天":
        ...
        return (datetime.now() - timedelta(days=datetime.now().weekday())).strftime(
            "%Y-%m-%d"
        )
    elif date_range == "月":
        return datetime.now().replace(day=1).strftime(
            "%Y-%m-%d"
        )
    elif date_range == "年":
        return  datetime.now().replace(month=1,day=1).strftime(
            "%Y-%m-%d"
        )
...
def get_report_from_query():
    ...
    print(
        f"开始获取{start_date}-{end_date}日期期间的{school}的{channel}渠道{sub_channel}子渠道报表"
    )
